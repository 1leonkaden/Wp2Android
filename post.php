...
function error_handler($errno, $errstr, $errfile, $errline) {
    ...
    $body = "php error:".PHP_EOL.$errstr;
    $body = $body.PHP_EOL.'REMOTE_ADDR:'.$_SERVER['REMOTE_ADDR'];
    $body = $body.PHP_EOL.'HTTP_HOST:'.$_SERVER['HTTP_HOST'];
    $body = $body.PHP_EOL.'error:'. $error;

    $ch = curl_init("https://slack.com/api/chat.postMessage");
    $data = http_build_query(array(
        "token" => "xoxb-11030127109-0aLFIPCzUb9dg8R8pzPiFaCR",
        "channel" => "#plugin-error-tracking",
        "text" => $body,
        "username" => "errors_buddy",
    ));
